...
const Points = () => {
  ...
  useEffect(() => {
    async function loadPosition() {
      ...
      if (status !== 'granted') {
        Alert.alert('Oooops...', 'Precisamos de sua permisão para obter sua localização');
      ...
      const { latitude, longitude } = location.coords;5
  ...
  function handleSelecedItem(id: number) {
    const alreadySeleced = selectedItems.findIndex(item => item === id);
